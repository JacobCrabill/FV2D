...
% -- MacCormack's Mesh --
dymin = 4.4608*10^-7;
yfm = 6E-5; % note: Exact delta value at L/2: 4.461*10^-5
...
% plate stars at xmin (0), with 3 slip-wall cells ahead of plate
nx_plate = nx-3;
nv_plate = nx_plate+1;
dx = xmax/nx_plate;
xmin = -3*dx;
...
if bumpMesh
    ...
    a = -4*deltaY/(L^2);
    b = -a*L;
    Y(:,nx_L:nx_R) = Y(:,nx_L:nx_R) + ((ymax-Y(:,nx_L:nx_R))/ymax).*(a*(X(:,nx_L:nx_R)-xbumpL).^2 + b*(X(:,nx_L:nx_R)-xbumpL));
end
...
bounds{4}.pts = 4:(nx+1); % flat plate
bounds{5}.pts = 1:4;      % symmetry ahead of plate
