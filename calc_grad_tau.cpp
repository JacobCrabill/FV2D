 ...
 *
 *		dUdX = calc_grad(U,v2c,v2nc,xv,e2v,v2ne,c2v,c2nv,gv2iv,gv2bc)
 *
                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  int n_fields, n_dims, n_verts, n_cells_tot, n_verts_tot, n_edges_tot;
  int n_ghost_verts;

  /* check for proper number of arguments */
  if(nrhs!=10) {
      mexErrMsgTxt("calc_grad_tau.cpp: 10 inputs required.");
  }
  if(nlhs!=1) {
      mexErrMsgTxt("calc_grad_tau.cpp: One output required.");
  }
  ...
  if(n_dims!=2) {
    ...
    mexErrMsgTxt("calc_grad_tau.cpp: Wrong number of physical dimensions found.");
  }
  
  if(n_fields!=6) {
    mexErrMsgTxt("calc_grad_tau.cpp: Expecting 6 fields - dudx,dudy,dvdx,dvdy,dTdx,dTdy.");
  }
  ...
  for (int iv=0; iv<n_ghost_verts; iv++) {
    ...
    if (bc<=4) // inlet/outlet - use ghost cell
    {
      // consider making more robust by taking avg of all cells touching vertex
      icg = v2c[ivg]-1;
      for (int f=0; f<n_fields; f++) {
        Uv[f+ivg*n_fields] = U[icg+f*n_cells_tot];
      }
    }
    else if (bc==5) // slip wall - flip dudy, dvdx, dTdy
    {
      Uv[0+ivg*n_fields] =  Uv[0+ivi*n_fields]; // dudx
      Uv[1+ivg*n_fields] = -Uv[1+ivi*n_fields]; // dudy
      Uv[2+ivg*n_fields] = -Uv[2+ivi*n_fields]; // dvdx
      Uv[3+ivg*n_fields] =  Uv[3+ivi*n_fields]; // dvdy
      Uv[4+ivg*n_fields] =  Uv[4+ivi*n_fields]; // dTdx
      Uv[5+ivg*n_fields] = -Uv[5+ivi*n_fields]; // dTdy
    }
    else if (bc==6 || bc==7) // no-slip wall - flip dudx, dvdx, dTdy
    {
      Uv[0+ivg*n_fields] = -Uv[0+ivi*n_fields]; // dudx
      Uv[1+ivg*n_fields] =  Uv[1+ivi*n_fields]; // dudy
      Uv[2+ivg*n_fields] = -Uv[2+ivi*n_fields]; // dvdx
  ...
  for (int ie=0; ie<n_edges_tot; ie++) {
