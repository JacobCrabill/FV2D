...
% Near-wall fine-mesh params
dymin = 1.0564E-5;
yfm = .005; % note: Exact delta value at L/2: 4.461*10^-5
nc_fm = 21; % # of cells in near-wall mesh

% Number of CELLS in mesh (NOT vertices)
nx = 100;
ny = 70;
nxbump = 40;
nxL = 30; % # of cells before bump

% Scaling parameters for bump mesh
gammaXL = 6.5; % Scaling/stretching parameter for X dir. to left of bump
gammaXR = 6.5; % Scaling/stretching parameter for X dir. to right of bump
gammaXB = .5; % Scaling/stretching parameter for X direction along bump
...
dxi = 1/nx;
deta = 1/ny;
[XI,ETA] = meshgrid(0:dxi:1,0:deta:1);

X = XI*(xmax-xmin) + xmin;

% Setup Bump
nxR = nxL+1+nxbump;
xbump = linspace(0,L,nxbump+1); %X(1,nxL+1:nxR);
ybump = sqrt(r^2-(xbump-xc).^2)+yc;

% ----------------- Scale the Mesh the McCormack Way -----------------
eta1 = linspace(0,1,nc_fm)';

% Scale the near-wall grid by varying gammaY1 until parameters are met
gammaY1 = 3;
Yfm = (yfm-ymin)*(1/(1-exp(gammaY1)))*(1-exp(gammaY1*eta1))+ymin;
Y_1 = Yfm(2)-Yfm(1);  % note that Yfm(1) should be 0
...
ETA1 = repmat(eta1,[1,nx+1]);
Yfm = (yfm-ymin)*(1/(1-exp(gammaY1)))*(1-exp(gammaY1*ETA1))+ymin;

% Set outer region params
dymin_cm = Yfm(nc_fm,1)-Yfm(nc_fm-1,1); % height of last cell in fine mesh
nc_cm = ny+1 - nc_fm + 1;
eta2 = linspace(0,1,nc_cm)';

% Scale the outer mesh by varying gammaY2 until params are met
gammaY2 = 1;
Ycm = (ymax-yfm)*(1/(1-exp(gammaY2)))*(1-exp(gammaY2*eta2))+yfm;
Y_2 = Ycm(3)-Ycm(2);
...
% --------------------------------- Bump! ---------------------------------
% Shift the bump up, with amount of shift decreasing with height
Y(:,nxL+1:nxR) = Y(:,nxL+1:nxR) + ((ymax-Y(:,nxL+1:nxR))/ymax).*repmat(ybump,[ny+1,1]);

% Additional setup for the x-direction scaling
nx_L = nxL+1;
nx_R = nxR;
nx_mid = nx_L+round(nxbump/2);
xiL = XI(1,nx_L);
xiR = XI(1,nx_L+nxbump);
xiMid = XI(1,nx_mid);
xMid = (xbumpR+xbumpL)/2;

% -------------------------- X-Direction Scaling --------------------------
% Left
X(:,1:nx_L) = (xbumpL-xmin)*(1/(1-exp(-gammaXL)))*(1-exp(-gammaXL*(XI(:,1:nx_L)/xiL)))+xmin;
% Bump
X(:,nx_L:nx_mid) = (xMid-xbumpL)*(1/(1-exp(gammaXB)))*(1-exp(gammaXB*(XI(:,nx_L:nx_mid)-XI(1,nx_L))/(xiMid-xiL)))+xbumpL;
X(:,nx_mid:nx_R) = (xbumpR-xMid)*(1/(1-exp(-gammaXB)))*(1-exp(-gammaXB*(XI(:,nx_mid:nx_R)-XI(1,nx_mid))/(xiR-xiMid)))+xMid;
% Right
X(:,nx_R:nx+1) = (xmax-xbumpR)*(1/(1-exp(gammaXR)))*(1-exp(gammaXR*(XI(:,1:nx_L)/xiL)))+xbumpR;

% Show the mesh vertices
if plotmesh
    plot(X,Y,'k*');
    xlim([-.04,.05]),ylim([0,.04]);
end
...
bounds{4}.pts = nx_L:nxR;               % bump
bounds{5}.pts = [1:nx_L,nxR:nx+1];      % symmetry ahead of/behind bump
